              ...
              book = Spreadsheet::Workbook.new
                ...
                @results.each_with_index do |r, i|
                  if r.date.between?(params[:start_date].to_date ,params[:end_date].to_date) && params[:gstr_type] == 'GSTR2'
                      if r.bill_type == 'b2b'
                          sheet1.insert_row(sheet1.last_row_index + 1, ["#{r.customer.gstin_no_reg}", "#{r.purchase_no}", "#{r.date}","#{r.invoice_value}", "#{r.customer.address}","Regular","#{r.tax_rate}","#{r.invoice_value}","#{r.total_invoice_value}"])
                              if r.invoice_value.blank?
                                r.invoice_value = 0
                              end
                             invoicetest = invoicetest + r.invoice_value.to_i
                      ...
                      if r.bill_type == 'b2cl'
                          sheet2.insert_row(sheet1.last_row_index + 1, ["#{r.customer.gstin_no_reg}", "#{r.purchase_no}", "#{r.date}","#{r.invoice_value}", "#{r.customer.address}","Regular","#{r.tax_rate}","#{r.invoice_value}","#{r.total_invoice_value}"])
                              if r.invoice_value.blank?
                                r.invoice_value = 0
                              end
                             invoicetest = invoicetest + r.invoice_value.to_i
